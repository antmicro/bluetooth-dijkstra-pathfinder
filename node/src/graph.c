...
int load_rtr(struct node_t graph[], uint8_t buff[], uint8_t size)
{
	...
	for (uint8_t j = 0; j < neighs_n; j++) {
		...
		uint16_t cost_to_neigh =
		    (cost_to_neigh_msb << 8) | cost_to_neigh_lsb;
		int err =
		    graph_set_cost_uni_direction(&graph[node_addr],
						 &graph[neigh_addr],
						 cost_to_neigh);
		if (err)
			return err;
			   ...
			   uint8_t len)
{
	...
	int err =
	    get_ptr_to_node_by_ble_addr(graph, identity_str, &common_self_ptr);
	if (err)
		return err;
	return 0;
}

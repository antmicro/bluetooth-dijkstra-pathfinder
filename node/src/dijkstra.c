 ...
 */
static uint8_t *trace_back(struct node_t *start_node, struct node_t *dst_node,
			   uint8_t * paths_len);
			   ...
			   struct node_t *start_node, struct node_t *dst_node)
{
	...
	while (!(smallest_td_node_found_code =
		...
		err =
		    recalculate_td_for_neighbours(smallest_td_node_container->
						  node);
		if (err)
			return err;
	...
	__ASSERT(err == 0,
		 "ERROR: Could not reset tentative_distances in the graph (err %d)\n",
		 err);
			   ...
			   uint8_t * paths_len)
{
	...
	__ASSERT(path != NULL,
		 "ERROR: Could not allocate memory for path tracing\n");
				  ...
				  uint8_t graph_size, sys_slist_t * lst)
{
	...
	for (uint8_t i = 0; i < graph_size; i++) {
		if (graph[i].reserved) {
			...
			__ASSERT(container != NULL,
				 "ERROR: Could not allocate memory for node container \n");

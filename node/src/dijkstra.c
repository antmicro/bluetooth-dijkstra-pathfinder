...
void recalculate_td_for_neighbours(uint8_t node_addr, struct node_t *graph)
{
	...
	for (uint8_t i = 0; i < current_node->paths_size; i++) {
		struct node_t *neighbour =
		    graph + ((current_node->paths + i)->addr);
		...
		if (!visited) {
			...
			node_t_tentative_distance_get(current_node,
						      &curr_node_td);
			...
			if (curr_node_td + cost_to_neighbour < neigh_td) {
				...
				if (err) {
					printk
					    ("ERROR: Could not set tentative_distance\n");
				}
			}
		}
	}
		     ...
		     uint8_t start_addr, uint8_t dst_addr, uint8_t * paths_len)
{
	...
	if (!path)
		return NULL;	//PZIE: hard fail instead
	...
	while (current_node->addr != start_addr) {
		...
		for (uint8_t i = 0; i < number_of_paths; i++) {
			...
			node_t_tentative_distance_get(checked_node,
						      &checked_node_td);
			       ...
			       uint8_t graph_size, sys_slist_t * lst)
{
	...
	for (uint8_t i = 0; i < MAX_MESH_SIZE; i++) {
		if (graph[i].reserved) {
			...
			if (!container) {
				printk
				    ("k_malloc failed in create_unvisited_list\n");
				return 1;
			}
...
void remove_unvisited_slist_member(sys_slist_t * lst, struct node_container
